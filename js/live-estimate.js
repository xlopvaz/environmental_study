...
async function getClimateNormal(endDate) {
  ...
  const cacheKey = "climateNormalCache";
  ...
  } catch (e) { /* caché corrupta ou inexistente, seguimos e recalculamos */ }
  ...
  const normal = yearlySums.reduce((a, b) => a + b, 0) / yearlySums.length;

  try {
    localStorage.setItem(cacheKey, JSON.stringify({ date: todayStr, value: normal }));
  } catch (e) { /* se falla o gardado en caché, non pasa nada */ }
